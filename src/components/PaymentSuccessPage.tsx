...
export default function PaymentSuccessPage({ payment_intent }: Props) {
  ...
  return (
    <main className="max-w-6xl flex flex-col gap-2.5 mx-auto p-10 text-white text-center border m-10 rounded-md border-black">
      {loading ? (
        <div>validating...</div>
      ) : id ? (
        <div className="mb-10">
          ...
          <div className="bg-white text-black p-2 rounded-md my-5 text-4xl font-bold mx-auto">
            ${amount / 100}
          </div>
          <div>Uid: {uid}</div>
          <div>Id: {id}</div>
